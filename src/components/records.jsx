...
export default function Records() {
  ...
  const records = assets?.homepage?.records;
  return (
    <section>
      <div className="flex w-full mb-8">
        <div className="grid grid-cols-3 w-full">{
            records?.map(({ id, icon, text, span }) => (
                <div className="flex flex-col items-center !text-dark dark:!text-white" key={id}>
                    <div className="icon" dangerouslySetInnerHTML={{ __html: icon }} />
                    <strong className="text-center text-xl text-pink-500">{span}</strong>
